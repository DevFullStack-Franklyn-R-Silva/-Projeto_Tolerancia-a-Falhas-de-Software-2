...
public class ServerUp {
	public static void main(String[] args) {
		//distribuição de parte da configuração arquitetural
		//Limite
		IManager compLimite = ComponentFactory.createInstance();
		
		//Conector
		ILimiteOps limiteOps;
		limiteOps = (ILimiteOps)compLimite.getProvidedInterface("ILimiteOps"); //Instancia do objeto que de fato é o conector 
		try {
			IConnSrv connSrv = new ConnectorRMIServidor(limiteOps);
			LocateRegistry.createRegistry(1099); //Criação da porta para levantar o servidor
